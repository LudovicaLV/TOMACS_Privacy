function imgTime = TuringFixedSimStoch(Astart, Bstart,systemSize, parameters, noise)
...
%     B = reshape(M(2,:), systemSize, systemSize);
    ...
    for t=0:dt:T
        ...
        Ap = A + dt*(AB - A - 12 + dA(2:end-1,2:end-1)) + noise * randn(1)* sqrt(dt);
        ...
        Bp = B + dt*(16 - AB + dB(2:end-1, 2:end-1))+ noise * randn(1)* sqrt(dt);
        Bp(Bp < 0) = 0;
