...
public class SignalStatistics2 {
	...
	public SignalStatistics2(int size) {
		this.size = size;
	...
	public SignalStatistics2(int size, int steps) {
		this.size = size;
		this.steps= steps;
	...
	public synchronized double[][] getVarianceTraj() {
		if (this.varianceTraj == null) {
			computeVarianceTraj();
		}
		return this.varianceTraj;
	}
	...
	private void computeVarianceTraj() {
		if (this.samplings == 0) {
			return ;
		}
		this.varianceTraj = new double[size][steps];
		for( int i=0 ; i<size ; i++ ) {
			for(int j=0;j<steps; j++){
            this.varianceTraj[i][j] = (this.squareTraj[i][j]/this.samplings)-Math.pow(this.averageTraj[i][j], 2); 
			}
		}
	}	
...
//	}
	...
	public synchronized double[][] getStandardDeviationTraj() {
		if (this.standardDeviationTraj == null) {
			computeStandardDeviationTraj();
		}
		return this.standardDeviationTraj;
	}
...
//	}
	...
	private void computeStandardDeviationTraj() {
		computeVarianceTraj();
		if (this.samplings == 0) {
			return ;
		}
		this.standardDeviationTraj = new double[size][steps];
		for( int i=0 ; i<size ; i++ ) {
			for(int j=0;j<steps; j++){
			this.standardDeviationTraj[i][j] = 1.96 * Math.sqrt((this.averageTraj[i][j])*(1 - this.averageTraj[i][j])/this.samplings);;
            }
		}
	}	
	
	private void computeConfIntervalTraj(int runs, double conf) {
		computeAverageTraj();
		if (this.samplings == 0) {
			return ;
		}
		this.confIntTraj = new double[size][steps];
		for( int i=0 ; i<size ; i++ ) {
			for(int j=0;j<steps; j++){
			this.confIntTraj[i][j] = conf * Math.sqrt((this.averageTraj[i][j])*(1 - this.averageTraj[i][j])/runs);
            System.out.println("ok " + i + " " + j);
			}
		}
	}	
	
	public synchronized double[][] getConfIntervalTraj(int runs, double conf) {
		if (this.confIntTraj == null) {
			computeConfIntervalTraj(runs, conf);
		}
		return this.confIntTraj;
	}
